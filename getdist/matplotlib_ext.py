...
class BoundedMaxNLocator(ticker.MaxNLocator):
    ...
    def tick_values(self, vmin, vmax):
        ...
        if cos_rotation > 0.05 and isinstance(self._formatter, ticker.ScalarFormatter) and len(locs) > 1:
            ...
            if (
                len(locs) < max(3, nbins)
                or step < label_len * (1.1 if len(locs) < 4 else 1.5)
                or (
                    locs[0] - vmin > min(step * 1.01, label_len * 1.5)
                    or vmax - locs[-1] > min(step * 1.01, label_len * 1.5)
                )
            ):
                ...
                if len(locs) <= 1 and size_ratio * self._font_aspect < 0.9:
                    ...
                    for sc in [scale, scale / 10.0]:
                        locs = [round((vmin * 3 + vmax) / (4 * sc)) * sc, round((vmin + 3 * vmax) / (4 * sc)) * sc]
    ...
    def _valid(self, locs):
        ...
        return (len(locs) < 2 or locs[1] - locs[0] > label_len * 1.1) and (
            not self.bounded_prune
            or (locs[0] - self._range[0] > label_len / 2)
                ...
                for i in reversed(range(istep + 1)):
                    ...
                    if min_ticks <= high - low + 1 <= nbins:
                        ...
                        if off and round_center and changing_lengths:
                            ...
                            if step > 2 * sc:
                                for shift in [0, -1, 1, -2, 2]:
                                    ...
                                    if any(np.round(shifted / sc / 10) * 10 == np.round(shifted / sc)) and self._valid(
                                        shifted
                                    ):
                                        ticks = shifted
                        ...
                        odd_gaps = min_ticks > 1 and (
                            ...
                            or self.bounded_prune
                            and (
                                ticks[0] - self._range[0] > max(min(_full_range / 3, step), label_len * 1.1)
                                or self._range[1] - ticks[-1] > max(min(_full_range / 3, step), label_len * 1.1)
                            )
                            or not self.bounded_prune
                            and len(ticks) == 3
                            and step > max(2 * label_len, _full_range / 3)
                            and step_int > 1
                            and round(ticks[-1] / sc) % 10 > 0
                        )
                        ...
                        too_few_points = (
                            len(ticks) < 3
                            and (nbins > (3 if step_ix else 4))
                            or (len(ticks) < max(2, (nbins + 1) // 2))
                        ) and step > label_len * 1.5
                        ...
                        if (
                            ...
                            and not step_ix
                            or big_step
                            and (not len(best) or len(ticks) < len(best))
                            or close_ticks
                            or too_few_points
                            or odd_gaps
                        ):
