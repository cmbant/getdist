    ...
    with st.sidebar:
        ...
        if st.session_state.chain_dir:
            ...
            if st.session_state.selected_roots:
                ...
                if not st.session_state.active_chain or st.session_state.active_chain not in st.session_state.selected_roots:
        ...
        with settings_col:
            ...
            if st.session_state.plot_type == "1D Density":
                st.session_state.plot_settings['normalized'] = st.checkbox(
                    "Normalize",
                    value=st.session_state.plot_settings.get('normalized', True)
                )
                ...
                if st.session_state.plot_settings['use_z_axis']:
                    ...
                    if z_params:
                        ...
                        st.session_state.plot_settings['shadows'] = st.checkbox(
                            "Shadows",
                            value=st.session_state.plot_settings.get('shadows', False)
                        )
                ...
                st.session_state.plot_settings['filled'] = st.checkbox(
                    "Filled Contours",
                    value=st.session_state.plot_settings.get('filled', True)
                )

                st.session_state.plot_settings['show_1d'] = st.checkbox(
                    "Show 1D Distributions",
                    value=st.session_state.plot_settings.get('show_1d', True)
                )
