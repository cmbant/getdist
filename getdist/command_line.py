...
def getdist_script(args, exit_on_error=True):
    ...
    for separator in ["_", "."]:
        chain_files = chains.chainFiles(
            in_root, first_chain=first_chain, last_chain=last_chain, chain_exclude=chain_exclude, separator=separator
        )
        if chain_files:
            break
...
def getdist_command(args=None):
    ...
    parser.add_argument(
        "ini_file", nargs="?", help=".ini file with analysis settings (optional, if omitted uses defaults)"
    )
    parser.add_argument(
        "chain_root",
        nargs="?",
    ...
    parser.add_argument(
        "--make_param_file", help="Produce a sample distparams.ini file that you can edit and use when running GetDist"
    )
